...
function BuildpackCommand (context, heroku, command, action) {
  ...
  this.remote = context.flags.remote || 'heroku'
  this.heroku = heroku
...
BuildpackCommand.prototype.displayUpdate = function (buildpacks) {
  if (buildpacks.length === 1) {
    ...
    cli.log(`Run ${cli.color.magenta('git push ' + this.remote + ' master')} to create a new release using this buildpack.`)
  } else {
    cli.log(`Buildpack ${this.action}. Next release on ${this.app} will use:`)
    this.display(buildpacks, '  ')
    cli.log(`Run ${cli.color.magenta('git push ' + this.remote + ' master')} to create a new release using these buildpacks.`)
  }
}
